...
\author{Rishabh~Tripathi~and~Saumya% TODO: Saumya's full name
\thanks{Manuscript prepared September 24, 2026, for the DA2 project review
and viva evaluation. \textit{(Corresponding author: Rishabh Tripathi.)}
Rishabh Tripathi (Roll No.~24BRS1136) and Saumya (Roll No.~24BRS1065) are
with the Department of Computer Science and Engineering, [Institution],
% TODO: replace [Institution] with the institution name
India. Course: BCSE306L --- Artificial Intelligence
...
\caption{End-to-end application flow. (1) The Gemini agent's only two tools
route through the protection layer: \texttt{read\_page} fetches the page and
receives only \texttt{safeContent} back, and \texttt{propose\_action} receives
the gate's binding decision. (2) The deterministic layer validates, normalises
the page into four content views, detects, scores, and partitions content.
(3) The multi-view fusion model of Fig.~\ref{fig:layer} evaluates the episode
and contributes an advisory verdict to the gate.}
\label{fig:overview}
\end{figure*}
...
The scan pipeline (Fig.~\ref{fig:overview}, middle band) executes five pure,
order-fixed stages:
