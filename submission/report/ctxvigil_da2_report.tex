...
% ============================================================================
% CtxVigil — DA2 Project Review Report (IEEE Transactions journal format,
% matching the reference report's first-page design)
%
% Compile on Overleaf:
...
%
% TODO for the authors (marked % TODO below): Saumya's full name and e-mail,
% and the institution name in the \thanks footnote.
% ============================================================================
...
% Reference-style running heads
\markboth{BCSE306L --- Artificial Intelligence: Final Project \& Viva Evaluation (DA2), 2026}%
{Tripathi and Saumya: CtxVigil: An Explainable Multi-View Protection Layer Against Indirect Prompt Injection}

% Reference-style keyword lead-in ("IEEE Keywords—")
\renewcommand{\IEEEkeywordsname}{IEEE Keywords}
...
\author{Rishabh~Tripathi~(24BRS1136)~and~Saumya~(24BRS1065)\\[4pt]
{\small Project guide: Vijayaprabhakaran~K}% TODO: Saumya's full name
\thanks{Manuscript prepared September 24, 2026, for the DA2 project review
and viva evaluation. The authors are with the Department of Computer Science
and Engineering, [Institution], India. % TODO: replace [Institution] with the institution name
Course: BCSE306L --- Artificial Intelligence
(e-mail: rishabh.j.tripathi2903@gmail.com).% TODO: second author's e-mail
}
\\[2.5ex]
\begin{minipage}{\textwidth}
{\small\mdseries\noindent\textbf{\itshape Abstract---}Large language model
(LLM) web agents read untrusted web content and take actions on the user's
behalf, which exposes them to \emph{indirect prompt injection}:
attacker-authored text, hidden in a page's visible text, DOM, HTML comments,
or accessibility tree, that instructs the agent to abandon its task and
perform a harmful action. This paper presents CtxVigil, a protection layer
that sits between an agent and the web and combines three complementary
...
\texttt{block} verdicts with withheld-content containment; (ii) an
independent action gate that blocks or escalates tool actions using task
alignment, risk-category postures, and scan evidence; and (iii) a dual-head
multi-view fusion network over MiniLM sentence embeddings of four
agent-context views (user task, system prompt, proposed action, observation)
plus a DeBERTa-v3 prompt-injection classifier. On a 20{,}000-sample benchmark
curated from InjecAgent- and BIPIA-style scenarios with hard negatives, the
fine-tuned DeBERTa-v3-small classifier reaches 100.00\% accuracy, precision,
recall, and F1 (ROC-AUC 1.000) on the 3{,}000-sample held-out test set; the
fusion network reaches 100\% F1 on 1{,}000 hold-out episodes. The rule-based
layer passes all seven contract fixtures and all six live agent episodes,
including a hidden-ARIA injection that a sighted reviewer cannot see, at
2--21\,ms per episode on a commodity CPU, fully offline and fail-closed.
\par\smallskip
\noindent\textbf{\itshape IEEE Keywords---}indirect prompt injection, LLM
agents, action gate, multi-view fusion, DeBERTa-v3, uncertainty estimation,
web security.}
\end{minipage}}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{fig1_proposed_architecture.png}
\caption{Overall architecture of the proposed CtxVigil system: the LLM web
agent's tool calls are routed through the protection layer, whose scan
pipeline, action gate, and advisory multi-view fusion model decide what the
agent may read and do.}
\label{fig:overview}
\end{figure*}
...
Fig.~\ref{fig:overview} shows the overall architecture of the deployed
system. An LLM web agent
...
\caption{Detailed data-flow of the protection layer. The agent runtime
supplies four context views: user task, system prompt, proposed action, and
observation. Tokenization and vectorization with the frozen all-MiniLM-L6-v2
encoder produce four 384-d embeddings, whose pairwise cosine similarities are
stored in the 4$\times$4 agreement matrix (in the InjecAgent hijack fixture the
task--action agreement S13 collapses from 0.911 to 0.004). In parallel, the
observation text passes through the fine-tuned DeBERTa-v3 classifier to yield
p(injection). The concatenated 1{,}543-d feature vector feeds the CtxVigil
fusion network, whose risk and uncertainty heads drive the thresholded
ALLOW / QUARANTINE / BLOCK decision that is returned to the agent runtime.}
\label{fig:layer}
\end{figure*}

\subsection{Scan Pipeline}
The scan pipeline executes five pure,
order-fixed stages:
...
The gate (Fig.~\ref{fig:overview}) is an \emph{independent} decision, never
...
\label{sec:fusion}
The learned component (Fig.~\ref{fig:layer}) treats an agent checkpoint as
...
\texttt{github.com/rishabhx29/}\allowbreak\texttt{Prompt-Injection-Detection-Model}.
